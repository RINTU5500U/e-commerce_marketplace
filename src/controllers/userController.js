...
module.exports = {
    ...
    fetchSeller: async (req, res) => {
        try {
            let { page } = req.params
            let findUser = await userModel.find({ userType: 'seller' }).select({ _id: 0, name: 1, __v: 0 }).skip(2 * ((page || 2) - 1)).limit(10)
    ...
    fetchSellerById: async (req, res) => {
        try {
            let { userId } = req.params
            let findUser = await userModel.findById(userId).select({ name: 1, __v: 0, _id: 0 })
            if (!findUser) {
